    ...
    Column(modifier = Modifier.fillMaxSize()) {
                ...
                Button(
                    ...
                    onClick = {
                        if (newCityName.isNotBlank() && newProvinceName.isNotBlank()) {
                            ...
                            selectedCity?.let { onUpdateCity(it, newCity) }
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            itemsIndexed(cities) { index, city ->
                CityRow(city = city, selectedCity, onSelectCity = {city -> selectedCity = city})
...
fun CityRow(city: City, selectedCity: City?, onSelectCity: (City?) -> Unit) {
    var rowColor by remember { mutableStateOf(Color.Transparent)}
    rowColor = if (selectedCity == city) Color.Gray else Color.Transparent
    ...
    Row(
        modifier = Modifier
            ...
            .clickable(
                onClick = {
                    if (selectedCity != city) onSelectCity(city) else onSelectCity(null)
                }
            )
    ) {
        Text(
